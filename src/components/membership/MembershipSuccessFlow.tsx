  ...
  if (state.kind === "missing") {
    return (
      <div className="rounded-xl border border-border-1 bg-glass-2 p-8">
        <h1 className="text-h2 font-semibold text-fg-1 md:text-h1">
          결제 정보를 확인할 수 없습니다
        </h1>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <Link
            href="/ai-lab"
            className="rounded-full border border-border-1 bg-glass-1 px-5 py-2.5 text-body font-medium text-fg-2 transition hover:border-border-2 hover:bg-glass-3"
          >
            AI 실험실
          </Link>
        </div>
  ...
  if (state.kind === "error") {
    return (
      <div className="rounded-xl border border-red-500/30 bg-status-danger/10 p-8">
        ...
        <p className="mt-3 text-body text-red-200">{state.message}</p>
        <p className="mt-2 text-meta text-red-300/80">
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-8">
        ...
        <h1 className="mt-4 text-h2 font-semibold text-fg-1 md:text-h1">
          환영합니다!
        </h1>
        <p className="mt-3 text-body leading-7 text-fg-2">
        ...
        {state.receiptUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-4 inline-block text-meta text-emerald-300 underline hover:text-emerald-200"
          >
            영수증 보기 →
          </a>
        )}
      ...
      <section className="rounded-xl border border-border-1 bg-glass-2 p-6 md:p-8">
        ...
        <ol className="mt-4 space-y-2 text-body text-fg-2">
          <li>
            <span className="mr-2 text-fg-4">1.</span>아래 버튼으로 Discord 서버
            입장
          </li>
          <li>
            <span className="mr-2 text-fg-4">2.</span>#역할-인증 채널에서 가입
            이메일을 입력
          </li>
          <li>
            <span className="mr-2 text-fg-4">3.</span>봇이 자동으로 멤버 역할을
            부여합니다
          </li>
        </ol>
      ...
      <section className="rounded-xl border border-border-1 bg-glass-1 p-6 text-meta leading-6 text-fg-3">
        <p className="font-medium text-fg-2">멤버 혜택 다시보기</p>

...
export function WinnerTopicHero({ topic, totalVotes }: WinnerTopicHeroProps) {
  ...
  return (
    <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-6 md:p-8">
      ...
      {topic.description && (
        <p className="mt-3 line-clamp-4 whitespace-pre-line text-sm leading-6 text-gray-300 md:text-base">
          {topic.description}
        </p>
      )}
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Link
          ...
          className="rounded-full bg-emerald-500/20 px-5 py-2.5 text-sm font-medium text-emerald-200 transition hover:bg-emerald-500/30"
        >
          투표 결과 보기
        </Link>
        <Link

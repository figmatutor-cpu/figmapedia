...
function validateParseResult(parseResult) {
  ...
  if (!parseResult.primaryNodeId) {
    validation.warnings.push(
      "No node-id found in URL - will need to use page root",
    );
  } else if (!parseResult.primaryNodeId.includes(":")) {
    validation.warnings.push(
      'Node ID format may be incorrect - expected format: "pageId:nodeId"',
    );
  }
  ...
  if (parseResult.fileType === "board") {
    validation.warnings.push(
      "FigJam boards may have different node structure than design files",
    );
  }

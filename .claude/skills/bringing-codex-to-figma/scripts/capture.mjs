  ...
  return new Promise((resolvePort, reject) => {
    ...
    server.on("error", (error) => {
      if (startPort >= 65535) {
        reject(
          new Error(
            `No free TCP port found from ${startPort} upward: ${error.message}`,
          ),
        );
        return;
      }
...
if (args["start-browser"]) {
  ...
  if (isDryRun) {
    ...
    console.log(
      "[dry-run]   Would start a headful Chromium browser on an auto-assigned CDP port.",
    );
    console.log(
      "[dry-run]   Would write .capture-browser.json with { cdpPort }.",
    );
    process.exit(0);
  }
  ...
  console.log(
    `[start-browser] Navigating to ${sbAppUrl} — log in if prompted (up to 2 min)...`,
  );
  await authPage.goto(sbAppUrl, { waitUntil: "domcontentloaded" });

  if (!authPage.url().startsWith(sbAppOrigin)) {
    try {
      await authPage.waitForURL((url) => url.startsWith(sbAppOrigin), {
        timeout: 120_000,
      });
      console.log("[start-browser] Back on app — session established.");
    } catch (_) {
      console.warn(
        "[start-browser] Timeout waiting for app root — continuing anyway.",
      );
    }
  } else {
...
const isLocal =
  /^(localhost|127(?:\.\d{1,3}){3}|0\.0\.0\.0|\[::1\]|.*\.local)$/.test(
    hostname,
  );
...
const stateViewNames = routePaths.filter(
...
if (hasStateViews) {
  ...
  console.log(
    `[views] Loaded navigate functions: ${Object.keys(viewsNavigate).join(", ")}`,
  );
  ...
  if (existsSync(prevSessionFile)) {
    try {
      ...
      if (staleIds.length > 0) {
        console.log(
          `[prepare] Closing ${staleIds.length} stale tab(s) from previous session...`,
        );
...
if (!hasHandover) {
  ...
  console.log(
    `[auth] Navigating to ${appUrl} — log in if prompted (up to 2 min)...`,
  );
  await authPage.goto(appUrl, { waitUntil: "domcontentloaded" });

  if (!authPage.url().startsWith(appOrigin)) {
    try {
      await authPage.waitForURL((url) => url.startsWith(appOrigin), {
        timeout: 120_000,
      });
...
if (!isLocal) {
  ...
  console.log(
    "[prepare] LOCAL — capture.js already in index.html, skipping inject.",
  );
}

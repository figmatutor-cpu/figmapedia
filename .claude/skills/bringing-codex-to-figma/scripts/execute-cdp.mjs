...
if (!viewName) {
  console.error(
    "Usage: node execute-cdp.mjs <viewName> <captureId> <figmaEndpoint>",
  );
...
try {
  ...
  console.error(
    "✗ .capture-session.json not found. Run capture.mjs --prepare first.",
  );
  process.exit(1);
}
  ...
  return new Promise((resolveResult, reject) => {
    ...
    ws.addEventListener("close", () => {
      ...
      if (pending.size > 0) {
        ...
        finishErr(
          new Error(
            `CDP socket closed before all responses arrived (pending ids: ${unresolved})`,
          ),
        );
        return;
      }
...
if (!closeOnly) {
  ...
  const commands = [
    { method: "Runtime.evaluate", params: { expression, awaitPromise: false } },
  ];
  ...
  if (evalResult?.result?.result?.subtype === "error") {
    console.error(
      "✗ captureForDesign threw:",
      evalResult.result.result.description,
    );
    process.exit(1);
  }
...
if (closeOnly) {
  try {
    const res = await fetch(
      `http://localhost:${cdpPort}/json/close/${target.id}`,
    );

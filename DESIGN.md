...
로컬 Pretendard Variable을 사용한다. 한국어 제목은 자간을 좁히고 22–64px의 반응형 크기로 구성한다. 본문은 14–17px, 행간은 1.8–1.9로 유지한다. 단어 단위 줄바꿈을 사용한다. 히어로는 중앙 정렬하며, “디자이너와 IT 실무자를 위한” 아래에 “AI+Figma 효율화 교육 → 팀 워크플로우 효율화 교육 → AI 업무 자동화 교육”을 순서대로 약 3초마다 롤링한다. Figma · Claude · ChatGPT · MCP · 바이브코딩을 실무 워크플로우에 연결한다는 설명을 붙인다.
...
44px 이상의 주요 클릭 영역, 명확한 키보드 포커스, 본문 바로가기, 모바일 메뉴의 상태 전달과 Escape 닫기, 이력 필터의 선택 상태, 검색 결과 수 알림을 제공한다. 히어로 롤링은 재생·일시정지를 제공하고, 모션 감소 설정에서는 첫 문구를 고정 표시한다. 스크린리더에는 전체 교육 문구를 정적 제목으로 제공한다.
...
홈의 교육 FAQ는 초기에는 모두 닫힌 아코디언이며 한 번에 한 항목만 열린다. 열린 질문을 다시 누르면 닫힌다. 교육 진행 방식, 기업·직무별 교육, Figma AI·MCP, 워크플로우 자동화 교육의 대상과 범위, 교육 후 추가 컨설팅을 설명한다. 공개 FAQ와 figmatutor.info 홈의 JSON-LD는 education-content.ts를 공유한다. 추천 순위나 성과를 만들어 넣지 않으며 실제 강의 이력과 커리큘럼으로 연결한다.

...
const records = [...history.lectures, ...history.communityEvents];
const categories = ["전체", ...new Set(records.map(lecture => lecture.category))];
...
export function LectureHistory() {
  ...
  const filtered = records.filter(lecture =>
  ...
  return <section id="experience" className={styles.section} aria-labelledby="experience-heading">
    ...
    <div className={styles.archive}>
      <div className={styles.archiveHeading}><h3>전체 강의·활동 이력</h3><span>{records.length}개 기록</span></div>
      <div className={styles.archiveBody}>
        ...
        <ul id="lecture-records" className={styles.records}>{visible.map(lecture => <li key={lecture.organization}><div><strong>{lecture.organization}</strong><span>{lecture.category}</span></div><span>{lecture.period === "미상" ? "시기 미기재" : lecture.period}</span><span>{lecture.frequency === "미상" ? (lecture.organization === "부산 인재원" ? "" : "횟수 미기재") : lecture.frequency.replace(" (최다)", "")}</span></li>)}</ul>
        ...
        <p className={styles.note}>조직별 강의와 커뮤니티·컨퍼런스 활동을 정리한 이력입니다. 횟수와 과정 표기는 진행 형태에 따라 다르며, 전체 강의 횟수를 의미하지 않습니다.</p>
      </div>
    </div>
  </section>;
}
